(function () {
  ...
  const API_KEY = "AIzaSyC2GNLPgeShHehkWs-0ODhyd6CEf-dGrPc";
  ...
  function addIncorrectAnswer(questionText, incorrectAnswerText) {
    ...
    console.log(`Added incorrect answer for question: "${questionText}": "${incorrectAnswerText}"`);
  }

  // Kiểm tra xem đáp án có được đánh dấu là sai từ trước không
  function isKnownIncorrectAnswer(questionText, answerText) {
    const data = getIncorrectAnswers();
    return data[questionText] && data[questionText].some(incorrect => 
  ...
  async function getAnswerFromGemini(questionText, optionTexts = [], incorrectOptions = [], isCheckbox = false) {
    try {
      console.log("Querying Gemini API for:", questionText);
      
      // Xác định số lượng lựa chọn cần chọn
      const requiredSelections = isCheckbox ? detectRequiredSelections(questionText) : 1;
      
      // Xây dựng prompt với các lựa chọn và đánh dấu những đáp án đã biết là sai
      let prompt = `Đây là một câu hỏi ${isCheckbox ? "nhiều lựa chọn" : "trắc nghiệm"}: ${questionText}\n\n`;
      ...
      if (optionTexts.length > 0) {
        ...
        optionTexts.forEach((option, index) => {
          ...
          prompt += `${option}${isIncorrect ? " (Đã biết là sai từ lần thử trước)" : ""}\n`;
        });
        ...
        if (isCheckbox) {
          ...
          if (incorrectOptions.length > 0) {
            prompt += "TRÁNH chọn các đáp án đã được đánh dấu là sai từ lần thử trước. ";
          }
          
          prompt += "Chỉ trả lời các chữ cái tương ứng với đáp án (ví dụ: A, C). Liệt kê bằng dấu phẩy nếu có nhiều đáp án. Không giải thích.";
        } else {
          ...
          prompt += "Chỉ đưa ra chữ cái nếu là đáp án A, B, C, D... hoặc đưa ra nội dung đáp án đúng. Không giải thích.";
        }
      } else {
        prompt += "Hãy trả lời câu hỏi này. Chỉ đưa ra đáp án, không giải thích.";
      }
  ...
  function getOptionText(optionElement) {
    ...
    const textContainer = optionElement.querySelector(".rc-CML [data-testid='cml-viewer']");
  ...
  function collectIncorrectAnswersFromResults() {
    ...
    const evaluatedQuestions = document.querySelectorAll('[data-testid^="part-Submission_"][data-testid$="Question"]');
    let collected = false;

    evaluatedQuestions.forEach(question => {
      // Tìm văn bản câu hỏi
      const promptElement = question.querySelector('[id^="prompt-autoGradableElementId"], [id^="prompt-autoGradableResponseId"]');
      if (!promptElement) return;
      
      const questionText = promptElement.textContent.trim();
      
      // Xử lý câu hỏi radio (MCQ)
      if (question.getAttribute('data-testid').includes('MultipleChoiceQuestion')) {
      ...
      // Xử lý câu hỏi checkbox (multiple select)
      else if (question.getAttribute('data-testid').includes('CheckboxQuestion')) {
        ...
        options.forEach(option => {
          ...
          const optionId = optionContainer ? optionContainer.getAttribute('data-testid') : null;
          const isIncorrect = question.querySelector(`.css-pn7qkz[data-for="${optionId}"]`);
  ...
  function checkAndRetryQuiz() {
    ...
    if (gradePercentage < 70) {
      ...
      const retryButton = document.querySelector('[data-testid="CoverPageActionButton"]');
      if (retryButton) {
        ...
        setTimeout(() => {
          const continueButton = document.querySelector('[data-testid="StartAttemptModal__primary-button"]');
          if (continueButton) {
            console.log("Clicking 'Continue' button in retry confirmation dialog...");
            continueButton.click();
          } else {
            console.warn("Continue button in retry confirmation dialog not found");
          }
        }, 1000);
  ...
  async function processMCQQuestions() {
    ...
    const questionContainers = document.querySelectorAll('[data-testid="part-Submission_MultipleChoiceQuestion"]');
    
    for (const container of questionContainers) {
      // Lấy prompt element chứa câu hỏi
      const promptElement = container.querySelector('div[id^="prompt-autoGradableResponseId"]');
      ...
      if (!optionsContainer) {
        console.warn("Không tìm thấy container đáp án (MCQ) cho câu hỏi:", questionText);
        continue;
      }

      // Lấy tất cả các option
      const optionElements = Array.from(optionsContainer.querySelectorAll('.rc-Option'));
      
      // Nếu không tìm thấy option nào
      if (!optionElements.length) {
        console.warn("Không tìm thấy đáp án nào cho câu hỏi (MCQ):", questionText);
        continue;
      }
      ...
      optionElements.forEach((option, index) => {
        ...
        if (isKnownIncorrectAnswer(questionText, optionText)) {
          ...
          console.log(`Option ${getOptionLabel(index)} is known to be incorrect:`, optionText);
        }
      });

      // Gọi Gemini API để lấy đáp án
      const answer = await getAnswerFromGemini(questionText, optionTexts, incorrectOptionIndices, false);
      
      if (!answer) {
        console.warn("Không nhận được đáp án từ Gemini cho câu hỏi (MCQ):", questionText);
        continue;
      }
      ...
      if (letterMatch) {
        ...
        if (letterIndex >= 0 && letterIndex < optionElements.length) {
          ...
          if (isKnownIncorrectAnswer(questionText, optionText)) {
            console.warn(`Gemini suggested option ${letter} but it's known to be incorrect:`, optionText);
            
            // Chọn một option khác không thuộc danh sách sai
            const availableOptions = optionElements.filter((_, index) => 
              !incorrectOptionIndices.includes(index)
            );
            ...
            if (availableOptions.length > 0) {
              ...
              if (input && !input.checked) {
                input.click();
                console.log(`Selected fallback option with text:`, getOptionText(fallbackOption));
              }
            } else {
              console.warn("No safe options available after filtering incorrect answers");
            }
          } else {
          ...
          console.warn(`Invalid option letter ${letter} for question:`, questionText);
        }
      } else {
        ...
        if (bestOption) {
          ...
          if (input && !input.checked) {
            input.click();
            console.log(`Clicked option ${getOptionLabel(bestOptionIndex)} with text:`, getOptionText(bestOption));
          }
        } else {
          console.warn("Không tìm thấy đáp án phù hợp sau khi lọc các đáp án sai:", questionText);
          
          // Nếu không tìm thấy đáp án phù hợp sau khi lọc, chọn đáp án đầu tiên không thuộc danh sách sai
          const safeOptions = optionElements.filter((_, index) => !incorrectOptionIndices.includes(index));
          if (safeOptions.length > 0) {
            const fallbackOption = safeOptions[0];
            const input = fallbackOption.querySelector("input");
            if (input && !input.checked) {
              input.click();
              console.log("Selected first safe option:", getOptionText(fallbackOption));
            }
          }
        }
      }
  ...
  async function processCheckboxQuestions() {
    ...
    const questionContainers = document.querySelectorAll('[data-testid="part-Submission_CheckboxQuestion"]');
    ...
    for (const container of questionContainers) {
      ...
      if (!optionsContainer) {
        console.warn("Không tìm thấy container đáp án (Checkbox) cho câu hỏi:", questionText);
        continue;
      }

      // Lấy tất cả các option
      const optionElements = Array.from(container.querySelectorAll('.rc-Option'));
      
      // Nếu không tìm thấy option nào
      if (!optionElements.length) {
        console.warn("Không tìm thấy đáp án nào cho câu hỏi (Checkbox):", questionText);
        continue;
      }
      ...
      if (selectedCount >= requiredSelections) {
        console.log(`Question already has ${selectedCount}/${requiredSelections} selections, skipping:`, questionText);
        continue;
      }
      ...
      // Gọi Gemini API để lấy đáp án
      const answer = await getAnswerFromGemini(questionText, optionTexts, incorrectOptionIndices, true);
      
      if (!answer) {
        console.warn("Không nhận được đáp án từ Gemini cho câu hỏi (Checkbox):", questionText);
        continue;
      }
      ...
      if (selectedLetters.length === 0) {
        console.warn("Couldn't parse any option letters from Gemini response:", answer);
        continue;
      }
      ...
      if (selectionCount < requiredSelections) {
        ...
        for (const option of safeOptions) {
          ...
          if (input && !input.checked) {
            input.click();
            console.log(`Selected additional safe option:`, getOptionText(option));
            selectionCount++;
          }
        }
      }
      
      console.log(`Selected ${selectionCount}/${requiredSelections} options for question.`);
    }
  }
  ...
  async function processInputQuestion(selector) {
    ...
    for (const container of containers) {
      ...
      if (!answer) {
        console.warn("Không nhận được đáp án từ Gemini cho câu hỏi (Input):", questionText);
        continue;
      }
    ...
    for (const selector of specificButtonSelectors) {
      ...
      if (specificButton) {
        ...
        return true;
          ...
          console.error(`Error clicking specific button with selector ${selector}:`, error);
        }
      }
    }
  ...
  function handleReadyToSubmitDialog() {
    ...
    const dialogSubmitButton = document.querySelector('button[data-testid="dialog-submit-button"]');
    ...
    const dialogSubmitButtonBackup = document.querySelector('.css-6z6oep button.cds-button-primary');
  ...
  async function processAllQuestions() {
    try {
      ...
      const readyToSubmitDialog = document.querySelector('[data-e2e="SubmitDialog__heading"]');
      ...
      const startAttemptDialog = document.querySelector('[data-testid="StartAttemptModal__heading"]');
      ...
      await processInputQuestion('div[data-testid="part-Submission_RegexQuestion"]');
      await processInputQuestion('div[data-testid="part-Submission_NumericQuestion"]');
      ...
      if (!submitClicked) {
        console.warn("Could not click submit button. Will retry in 1 second...");
      ...
      setTimeout(() => {
        ...
        if (document.querySelector('[data-e2e="SubmitDialog__heading"]')) {
          ...
          if (dialogSubmitButton) {
            ...
            console.warn("Dialog Submit button không tìm thấy, sẽ tìm các nút tương tự...");
            ...
            const confirmButtons = Array.from(document.querySelectorAll('button')).filter(btn => {
              ...
              return text.includes('confirm') || text.includes('submit') || 
                     label.includes('confirm') || label.includes('submit');
            });
