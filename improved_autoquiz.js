...
(function () {
  ...
  function findAndClickResumeButton() {
    log("Đang tìm nút Resume/Edit...", "info");

    // Tìm nút Edit dựa trên data-testid="CoverPageActionButton" và text "Edit"
    const editButton = Array.from(
      document.querySelectorAll('button[data-testid="CoverPageActionButton"]')
    ).find((btn) => btn.textContent.trim().toLowerCase().includes("edit"));

    if (editButton) {
      log("Tìm thấy nút Edit, đang click...", "success");
      setTimeout(() => {
        log("Đang click nút Edit...", "info");
        editButton.click();
    ...
    const exactResumeButton = allButtons.find(
      (btn) => btn.textContent.trim().toLowerCase() === "resume"
    );
    ...
    // Tìm kiếm nút Edit dựa trên nội dung
    const exactEditButton = allButtons.find(
      (btn) => btn.textContent.trim().toLowerCase() === "edit"
    );

    if (exactEditButton) {
      log("Tìm thấy nút chính xác với text 'Edit'", "success");
      setTimeout(() => {
        log("Đang click nút Edit...", "info");
        exactEditButton.click();
      }, 1000);
      return true;
    }
    ...
    if (actionContainer) {
      const actionButton = actionContainer.querySelector("button");
      if (
        actionButton &&
        (actionButton.textContent.toLowerCase().includes("resume") ||
          actionButton.textContent.toLowerCase().includes("edit"))
      ) {
        log("Tìm thấy nút Resume/Edit trong container action", "success");
        setTimeout(() => {
          log("Đang click nút trong container...", "info");
          actionButton.click();
        }, 1000);
        return true;
      }
    ...
    if (
      buttonByClass &&
      (buttonByClass.textContent.toLowerCase().includes("resume") ||
        buttonByClass.textContent.toLowerCase().includes("edit"))
    ) {
      log("Tìm thấy nút Resume/Edit bằng class", "success");
      setTimeout(() => {
        log("Đang click nút bằng class...", "info");
        buttonByClass.click();
      }, 1000);
      return true;
    }

    // Cuối cùng, tìm nút có chứa từ "Resume" hoặc "Edit"
    const entryButton = allButtons.find(
      (btn) =>
    ...
    if (entryButton) {
      log("Tìm thấy nút có chứa text 'Resume' hoặc 'Edit'", "success");
      setTimeout(() => {
        log("Đang click nút...", "info");
        entryButton.click();
      }, 1000);
      return true;
    }
    ...
    log("Không tìm thấy nút Resume hoặc Edit, các nút trên trang:", "error");
  ...
  function checkAndRetryQuiz() {
    ...
    if (gradePercentage < passThreshold) {
      ...
      // Tìm và click nút retry hoặc edit
      const retryButton = document.querySelector(
        '[data-testid="CoverPageActionButton"]'
      );

      if (retryButton) {
        const buttonText = retryButton.textContent.trim().toLowerCase();
        log(`Đã tìm thấy nút action: "${buttonText}"`, "info");

        // Kiểm tra xem đây là nút Edit hay Retry
        if (buttonText.includes("edit")) {
          log("Đang click nút Edit để làm lại quiz...", "info");
        } else {
          log("Đang click nút Retry để làm lại quiz...", "info");
        }

        retryButton.click();

        // Xử lý dialog xác nhận nếu xuất hiện (chỉ áp dụng cho nút Retry)
        setTimeout(() => {
          // Chỉ cần tìm và xử lý dialog nếu đây là nút Retry, không phải Edit
          if (!buttonText.includes("edit")) {
            ...
            if (continueButton) {
              log(
                "Đang click nút 'Continue' trong hộp thoại xác nhận...",
                "info"
              );
              continueButton.click();
            } else {
              log(
                "Không tìm thấy nút 'Continue' trong hộp thoại xác nhận, hoặc đây là nút Edit nên không cần xác nhận",
                "info"
              );
            }
          }
        ...
        // Fallback: tìm kiếm theo text và các thuộc tính khác
        const allButtons = document.querySelectorAll("button");
        const editOrRetryButton = Array.from(allButtons).find((btn) => {
        ...
        if (editOrRetryButton) {
          log(
            `Đã tìm thấy nút ${editOrRetryButton.textContent.trim()} bằng phương pháp fallback, đang click...`,
            "info"
          );
          editOrRetryButton.click();
          return true;
        }

        log("Không tìm thấy nút retry hoặc edit", "warning");
      }
    } else {
      log(
  ...
  // Hàm xử lý dialog cảnh báo "Incomplete or invalid answers"
  function handleIncompleteAnswersDialog() {
    // Tìm dialog với nội dung cụ thể về câu trả lời không đầy đủ
    const dialogContent = document.querySelector(
      ".css-4s48ix, [id='core-dialog-content-content']"
    );

    if (
      dialogContent &&
      dialogContent.textContent.includes(
        "We found some incomplete or invalid answers"
      )
    ) {
      log(
        "Phát hiện dialog 'Incomplete answers'. Quay lại để hoàn thành quiz...",
        "warning"
      );

      // Tìm nút "Cancel" hoặc nút thứ hai trong dialog
      const buttons = document.querySelectorAll("button");
      let cancelButton = null;

      // Tìm nút Cancel theo text
      for (const button of buttons) {
        const text = button.textContent.trim().toLowerCase();
        if (text === "cancel" || text === "go back" || text === "no") {
          cancelButton = button;
          break;
        }
      }

      // Nếu không tìm thấy theo text, thử tìm nút secondary (thường Cancel là nút secondary)
      if (!cancelButton) {
        cancelButton = document.querySelector(
          "button.cds-button-secondary, button[data-testid='dialog-secondary-button']"
        );
      }

      if (cancelButton) {
        log("Đã tìm thấy nút 'Cancel', click để quay lại làm tiếp...", "info");
        cancelButton.click();
        return true;
      } else {
        log(
          "Không tìm thấy nút 'Cancel' trong dialog incomplete answers",
          "error"
        );
        return false;
      }
  ...
  async function processAllQuestions() {
    try {
      ...
      if (checkAndRetryQuiz()) {
        log("Đã bắt đầu làm lại quiz. Đợi trang tải lại...", "warning");
        // Chờ một khoảng thời gian cho trang tải lại
        setTimeout(processAllQuestions, 3000); // Gọi lại chính nó để xử lý quiz mới
        return;
      }
      ...
      if (startAttemptDialog) {
        ...
        if (continueButton) {
          ...
          setTimeout(processAllQuestions, 3000); // Gọi lại chính nó
          return;
        }
      }
      ...
      // Chờ một chút để đảm bảo các thay đổi DOM (chọn đáp án) được áp dụng
      ...
      log("Đang tìm ô đồng ý (agreement checkbox)...", "info");
      ...
      const agreementSelectors = [
        ...
        'input[type="checkbox"][aria-labelledby*="check-agree"]',
      ];
      ...
      for (const selector of agreementSelectors) {
        ...
        if (checkbox) {
          ...
          log(`Đã tìm thấy ô đồng ý với selector: ${selector}`, "success");
          break;
        }
      }
      ...
      log(`Tìm thấy ${allCheckboxes.length} checkbox trên trang:`, "info");
      Array.from(allCheckboxes).forEach((cb, index) => {
        log(
          `Checkbox #${index}: id=${cb.id}, visible=${
      ...
      // If we found the checkbox, try different methods to click it
      if (agreementCheckbox) {
        if (!agreementCheckbox.checked) {
          log("Đang cố gắng click vào ô đồng ý...", "info");

          try {
            // Method 1: Direct click
            agreementCheckbox.click();
            log("Đã click vào ô đồng ý (method 1)", "success");
          } catch (error) {
            log(`Lỗi khi click method 1: ${error}`, "error");
            ...
            try {
              ...
              agreementCheckbox.dispatchEvent(clickEvent);
              log("Đã gửi sự kiện click cho ô đồng ý (method 2)", "success");
            } catch (error2) {
              log(`Lỗi khi click method 2: ${error2}`, "error");

              try {
                // Method 3: Change checked property directly
                agreementCheckbox.checked = true;
                agreementCheckbox.dispatchEvent(
                  new Event("change", { bubbles: true })
                );
                log(
                  "Đã thay đổi trực tiếp thuộc tính checked (method 3)",
                  "success"
                );
              } catch (error3) {
                log(`Lỗi khi thực hiện method 3: ${error3}`, "error");
              }
            }
          }
          ...
          setTimeout(() => {
            log(
              `Trạng thái ô đồng ý sau khi click: checked=${agreementCheckbox.checked}`,
              "info"
            );
          }, 500);
        } else {
        ...
        log(
          "Không tìm thấy ô đồng ý với các selector thông thường. Thử phương pháp dự phòng...",
          "warning"
        );

        // Fallback: Try to find and click the label or parent elements
        const honorCodeLabel = Array.from(
          document.querySelectorAll("label")
        ).find(
          (label) =>
            label.textContent.includes("honor code") ||
            label.textContent.includes("understand that submitting work")
        );

        if (honorCodeLabel) {
          log("Đã tìm thấy label honor code, thử click...", "info");
          honorCodeLabel.click();
        }
      }

      // Chờ một chút sau khi tick ô đồng ý
      ...
      const maxAttemptsToFindSubmit = 10; // Chờ tối đa 10 * 1 giây = 10 giây
      ...
      if (submitButton && !submitButton.disabled) {
        ...
        // Thêm một kiểm tra ngay sau khi click nút submit
        setTimeout(() => {
          if (handleIncompleteAnswersDialog()) {
            log(
              "Đã xử lý dialog 'Incomplete answers' ngay sau khi submit",
              "success"
            );
            // Tiếp tục xử lý các câu hỏi sau khi xử lý dialog
        ...
        // Có thể thêm logic fallback ở đây nếu cần, ví dụ thử lại sau một thời gian dài hơn hoặc báo lỗi
      }

      // Sau 2 giây, check và xử lý dialog "Ready to submit?"
      setTimeout(() => {
        // Kiểm tra xem có dialog cảnh báo câu trả lời không đầy đủ không
        if (handleIncompleteAnswersDialog()) {
          log(
            "Đã xử lý dialog 'Incomplete answers', quay lại làm tiếp quiz",
            "success"
          );
          // Tiếp tục xử lý các câu hỏi sau khi xử lý dialog
          setTimeout(processAllQuestions, 2000);
        ...
        if (document.querySelector('[data-e2e="SubmitDialog__heading"]')) {
          log("Đã tìm thấy dialog 'Ready to submit?' sau khi chờ", "info");
          handleReadyToSubmitDialog();
        } else {
          ...
          if (dialogSubmitButton) {
            ...
            log(
              "Không tìm thấy nút Submit trong dialog, đang tìm các nút tương tự...",
              "warning"
            );
            ...
            if (confirmButtons.length > 0) {
              log("Đã tìm thấy nút confirm tiềm năng, đang click...", "info");
